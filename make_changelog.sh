...
old_number_commits=$(($(grep "rev" < OPL_DETAILED_CHANGELOG | head -1 | cut -d " " -f 1 | cut -c 4-)))
number_commits=$(wc -l < /tmp/commit_summary)
new_number_commits=$((number_commits - old_number_commits + 2))

# Echo it!
echo "Current Revision ${new_number_commits}"
...
do
 ...
 echo "rev${new_number_commits} - ${line_commit_summary}" >> /tmp/commit_summary_new
